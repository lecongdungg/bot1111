...
def handle_user_request(user_id, total_run_time, message):
    interval = 20
    logging.info("Lấy được ID người dùng: %s và thời gian chạy: %d giây",
                 user_id, total_run_time)
    ...
    for i, line in enumerate(data):
        if line.strip().startswith('referrer ='):
            ...
            logging.info("Đã thay thế ID referrer ở dòng %d trong file %s", i,
                         temp_script)
    ...
    for _ in range(num_intervals):
        process = subprocess.Popen(['python', temp_script],
        ...
        if stdout:
            logging.info("Kết quả đầu ra của script %s: %s", temp_script,
                         stdout.decode())
        if stderr:
            logging.error("Kết quả lỗi của script %s: %s", temp_script,
                          stderr.decode())
...
def handle_id(message):
    try:
        ...
        if total_run_time > 1000:
            bot.reply_to(
                message,
                "Thời gian chạy không được vượt quá 1000 giây. Vui lòng thử lại với thời gian ngắn hơn."
            )
            return
        ...
        if user_id in running_ids:
            bot.reply_to(
                message,
                f"ID: {user_id} đang chạy. Vui lòng đợi cho đến khi hoàn thành trước khi yêu cầu tiếp."
            )
            return
        ...
        user_thread = threading.Thread(target=handle_user_request,
                                       args=(user_id, total_run_time, message))
        user_thread.start()
